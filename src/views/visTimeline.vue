...
function drawLines() {
  ...
  Object.values(timeline.itemSet.items).forEach((item) => {
    ...
    // 只处理选中的炉次
    if (item.data.heatId !== selectedHeatId) return
...
function highlightHeat(props) {
  ...
  // 如果点击的是同一个炉次，则取消选中
  if (selectedHeatId === clicked.heatId) {
    selectedHeatId = null
    // 恢复所有项目不透明度
    // document.querySelectorAll('.vis-item').forEach((el) => (el.style.opacity = ''))
    // 清空连线
    drawLines()
    return
  }
